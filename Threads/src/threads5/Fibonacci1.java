 ...
 * @author Daniel
 */
public class Fibonacci1 implements Supplier<BigInteger>{
    ...
    public Fibonacci1(BigInteger number) {
        this.number=number;
    ...
    public static BigInteger calculateRecursively(BigInteger number){
        if (number.compareTo(BigInteger.ONE)==0||number.compareTo(BigInteger.ZERO)==0){
            ...
            return calculateRecursively(number.subtract(BigInteger.ONE)).add(calculateRecursively(number.subtract(Fibonacci1.TWO)));
        }
    }
